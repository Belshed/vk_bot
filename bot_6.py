...
class User():
    ...
    def save(self, url):
        ...
        logger.info("Данные отправлены: " + data)
                ...
                if event.text == "/start" or event.text == self.retry_msg or event.text == self.retry_always or event.text == "Начать":
                    ...
                    if next_step == "doctor":
                        ...
                        try:
                            ...
                            try:
                                ...
                                message = "Выберите, пожалуйста, с имеющихся вариантов"
                    ...
                    if self.clients[event.user_id].next_step == "time":
                        ...
                        try:
                            ...
                            message = "К сожалению, в %s к %s больше нет свободного времени" % (self.clients[event.user_id].day, self.clients[event.user_id].doctor)
                            keyboard = self.get_specializations_keyboard().get_keyboard()
                            self.clients[event.user_id].go_to_specialization()
